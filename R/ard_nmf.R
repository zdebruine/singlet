...
#' @param k_min minimum rank to consider during automatic rank determination (cannot be less than 2)
#' @details
#'
...
#' @export
#'
ard_nmf <- function(A, k_init = 2, k_max = 100, k_min = 2, n_replicates = 1, tol = 1e-5, cv_tol = 1e-4,
  ...
  for (curr_rep in 1:n_replicates) {
    ...
    step_size <- 1
    curr_rank <- k_init
    while (step_size >= 1 && curr_rank <= k_max && curr_rank >= k_min) {
  ...
  if (!sparse_list) {
    if (dense_mode) {
      ...
      model <- c_nmf(A, At, tol, maxit, verbose > 2, L1, L1, L2, L2, threads, w_init_this)
    }
  } else {
