...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "singlet", "tests")); system.time(test_check("singlet", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran singlet_$(VERSION).tar.gz
	rm -rf singlet.Rcheck/
