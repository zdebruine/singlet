...
template <typename T>
void predict(spmat A, const Eigen::Matrix<T, -1, -1>& w, Eigen::Matrix<T, -1, -1>& h, const T L1, const T L2, const int threads) {
...
template <typename T>
void predict_link(spmat A, const Eigen::Matrix<T, -1, -1>& w, Eigen::Matrix<T, -1, -1>& h, const T L1, const T L2, const int threads,
...
void predict_mask(spmat A, const uint32_t seed, const uint32_t inv_density, const Eigen::Matrix<T, -1, -1>& w,
                  Eigen::Matrix<T, -1, -1>& h, const T L1, const T L2, const int threads, const bool mask_t) {
    Eigen::Matrix<T, -1, -1> a = AAt(w);
...
Rcpp::List run_nmf(const Rcpp::S4& A_, const Rcpp::S4& At_, const T tol, const uint16_t maxit, const bool verbose,
                   const T L1, const T L2, const uint16_t threads, Eigen::Matrix<T, -1, -1> w, Eigen::Matrix<T, -1, -1> link_h,
    ...
    for (uint16_t iter_ = 0; iter_ < maxit && tol_ > tol; ++iter_) {
        ...
        if (linking_h) {
            predict_link(A, w, h, L1, L2, threads, link_h);
        } else if (masking) {
        ...
        if (linking_w) {
            predict_link(At, h, w, L1, L2, threads, link_w);
        } else if (masking) {
...
Rcpp::List c_nmf(const Rcpp::S4& A_, const Rcpp::S4& At_, const double tol_, const uint16_t maxit, const bool verbose,
                 const double L1_, const double L2_, const uint16_t threads, Rcpp::NumericMatrix w_, Rcpp::NumericMatrix link_h_, Rcpp::NumericMatrix link_w_,
    ...
    if (float_precision) {
        ...
        model = run_nmf(A_, At_, tol, maxit, verbose, L1, L2, threads, w, link_h, link_w, seed, inv_density);
    } else {
        ...
        model = run_nmf(A_, At_, tol_, maxit, verbose, L1_, L2_, threads, w, link_h, link_w, seed, inv_density);
    }
    return model;
}