...
Now run NMF cross-validation and learn the model at the best rank:

```{R, cross-validation}
set.seed(123)
pbmc3k <- pbmc3k %>% NormalizeData() %>%
  singlet::RunNMF(k = 5:25, reps = 3, verbose = FALSE)
```
...
```{R, fig.width = 5, fig.height = 7, gsea-heatmap}
...
```{R, message = FALSE, warning = FALSE, cluster-cells}
